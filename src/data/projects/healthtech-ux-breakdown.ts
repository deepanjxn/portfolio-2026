import { ProjectDetail } from "@/types";

export const healthtechUxBreakdownProject: ProjectDetail = {
  slug: "healthtech-ux-breakdown",
  title: "GROWTHX",
  description:
    "A UX assignment completed as part of a product design interview process. Walkthrough of the thinking, decisions, and deliverables.",
  hero: "/projects/healthtech-ux-breakdown/background.webp",
  heroType: "vimeo",
  heroVideoId: "1209828074",
  type: "ux-breakdown",
  responsibilities: [],
  contribution: "",
  aboutProject:
    "GrowthX asked me to design a mobile experience for athletes preparing for endurance events such as Ironman and Ultraman. The assignment focused on helping users plan their training, monitor progress, stay motivated through community engagement, and access educational content within a single product. Rather than designing isolated screens, I approached the assignment as a complete product experience by defining the information architecture, planning the onboarding journey, and creating a scalable foundation before moving into interface design.",
  dateCreated: "28 May 2025",
  projectDuration: "2 Days",
  liveLink: "https://growthx.club/",
  sections: [
    {
      type: "editorial",
      title: "Problem Statement",
      body: "Design a community driven training platform that helps endurance athletes prepare for long distance events. The product needed to support personalised training plans, performance tracking, community participation, premium educational content, and motivation through progress based experiences. The solution also needed to accommodate users at different experience levels, from beginners preparing for their first race to advanced athletes working towards competitive goals.",
    },
    {
      type: "editorial",
      title: "My Approach",
      body: "I started by documenting the assignment requirements and converting the brief into a structured working document. This helped me identify assumptions, define the product scope, and understand what success should look like before designing anything. Once the problem was clear, I mapped the user journey, explored low fidelity sketches, and gradually refined the experience into high fidelity interfaces. Working this way allowed me to validate the overall product structure before focusing on visual design.",
    },
    {
      type: "editorial",
      title: "Key Design Decisions",
      body: "Instead of treating the application as a workout tracker, I positioned it as a complete training companion. The experience combines personalised plans, readiness insights, community engagement, educational content, and progress tracking within a single ecosystem. I also introduced personalised onboarding based on athlete goals and event types, allowing the application to adapt the experience from the very beginning instead of presenting the same interface to every user.",
    },
  ],
};
